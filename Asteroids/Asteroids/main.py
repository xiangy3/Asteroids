...
DT = 0.02						# seconds [virtual] for each frame
AI_TIME = 0.05					# time allowed for AI

INIT_NUM_ASTEROIDS = 8			# number of asteroids to start game
...
class AsteroidsDemo(ShowBase):
	...
	#	self.scoreBrd = genLabelText("Test", 0)
		...
		self.myshipAI = myShipAI(FIELD_SZ, AI_TIME, TURN_RATE,
							BULLET_SPEED, BULLET_REPEAT, BULLET_RAD,
							AST_SPEEDS, AST_DIAMS, NUM_FRAMES, PTS, SHIP_RAD)
		self.currFrame = 0
		self.currGame = 0
		self.totalScore = 0
		self.bullets = []
	...
	def gameLoop(self, task):
		...
		cmd = self.myshipAI.getAICommand(rocks, normalizeDegrees(self.ship.getH()+90), self.framesToFire)
